...
function loadStyleBoards() {
  try {
    const saved = localStorage.getItem(STYLE_BOARDS_KEY);
    if (saved) return JSON.parse(saved);
  } catch {}
  return DEFAULT_STYLE_BOARDS;
}

export function saveStyleBoards(boards) {
  try { localStorage.setItem(STYLE_BOARDS_KEY, JSON.stringify(boards)); } catch {}
}
